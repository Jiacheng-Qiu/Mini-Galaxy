...
public class BioRandomize : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        GenerateType(plantFolder, trees, 200, true, new Vector2(2, 6), 5);
        if (enableRock)
            GenerateType(plantFolder, rocks, 100, false, new Vector2(1, 1), 0);
        if (enableGrass)
            GenerateType(plantFolder, grass, 500, false, new Vector2(5, 8), 3);
        ...
        GenerateType(animalFolder, animals, 200, false, new Vector2(1, 1), 5);
    }

    // Build any type of creatures based on needs
    public void GenerateType(Transform parentFolder, Object[] prefab, int amount, bool resize, Vector2 groupSize, float randomRange)
    {
